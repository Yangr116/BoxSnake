...
def rasterize_instances(rasterizer, instances, shape, offset=0.0):
    if shape[0] != shape[1]:
        raise ValueError("expected square") # 为什么只支持正方形 ？？？
...
def sample_square(bs=1, max_inst=1, count=32, device="cpu"):
    """
    outputs: shape=(bs, max_inst, num_points, 2)
    """
    n_inter_pnts = count // 4 - 1
    pnts_base = torch.linspace(0, 1, n_inter_pnts+2, device=device) #
    grid_y, grid_x = torch.meshgrid(pnts_base, pnts_base)
    grid = torch.cat([grid_x.unsqueeze(-1), grid_y.unsqueeze(-1)], dim=-1)
    pnts_a_b = grid[0, :, :]
    pnts_b_c = grid[:, n_inter_pnts+1, :]
    pnts_c_d = grid[n_inter_pnts+1, :, :].flip(dims=[0])
    pnts_d_a = grid[:, 0, :].flip(dims=[0])
    pnts = torch.cat([pnts_a_b[:-1], pnts_b_c[:-1], pnts_c_d[:-1], pnts_d_a[:-1]], dim=0) # shape = (num_points, 2)
    pnts = pnts.clamp(min=0.00001, max=0.99999)
    return pnts[None, None, :, :].repeat(bs, max_inst, 1, 1)