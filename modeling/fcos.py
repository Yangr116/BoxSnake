# register the FCOS as a proposal generator
# yr copy from adelaidet
...
from .utils.comm import compute_locations, reduce_sum, reduce_mean, compute_ious, compute_ciou
from .utils.iou_loss import IOULoss
from .utils.ml_nms import ml_nms
...
class FCOSOutputs(nn.Module):
    ...
    def _get_ground_truth(self, locations, gt_instances):
        ...
        # debug
        # import copy
        # training_targets["reg_targets_wo_scale"] = copy.deepcopy(training_targets["reg_targets"])
    ...
    def get_sample_region(self, boxes, strides, num_loc_list, loc_xs, loc_ys, bitmasks=None, radius=1):
        """
        boxes shape = (num_gt, 4)
        """
        if bitmasks is not None:
            _, h, w = bitmasks.size() # shape=(num_gt, H, W)

            ys = torch.arange(0, h, dtype=torch.float32, device=bitmasks.device) # shape=(h)
            xs = torch.arange(0, w, dtype=torch.float32, device=bitmasks.device) # shape=(w)

            m00 = bitmasks.sum(dim=-1).sum(dim=-1).clamp(min=1e-6) # shape=(num_gt)
            m10 = (bitmasks * xs).sum(dim=-1).sum(dim=-1) # shape=(num_gt)
            m01 = (bitmasks * ys[:, None]).sum(dim=-1).sum(dim=-1) # 在 h 方向上乘 xs 
            center_x = m10 / m00
            center_y = m01 / m00
        else:
        ...
        K = len(loc_xs)
        boxes = boxes[None].expand(K, num_gts, 4) # shape=(K, num_gts, 4)
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None):
        ...
        instances.reg_targets = cat([
            ...
            x.reshape(-1, 4) for x in training_targets["reg_targets"] # 这里的 (l,t,r,b) 是按照那种方式解耦过来的？
        ], dim=0,)
        # debug
        # instances.reg_targets_wo_scale = cat([
        #     # Reshape: (N, Hi, Wi, 4) -> (N*Hi*Wi, 4)
        #     x.reshape(-1, 4) for x in training_targets["reg_targets_wo_scale"] # 这里的 (l,t,r,b) 是按照那种方式解耦过来的？
        # ], dim=0,)
        
        instances.locations = cat([
            x.reshape(-1, 2) for x in training_targets["locations"] # 这里的 location 是 (x, y)
        ], dim=0)
    ...
    def fcos_losses(self, instances):
        ...
        # compute cious, 这里有一种取巧的方法 将 (0, 0) 作为 location, 直接在对应的尺度下计算，但是可能存在未知的 bug
        # step1: convert the ltrb to x1y1x2y2
        # step2: compute cious
        _stride = 8 * (2 ** (instances.fpn_levels.unsqueeze(-1))) # 这里的 fpn level 是从 0 开始的，所以不能 -1 
        _reg_targets = instances.reg_targets * _stride # shape: (N, 4) * (N, 1) -> (N, 4)
        _reg_pred = instances.reg_pred * _stride
        _loc = instances.locations
        reg_t_x1y1x2y2 = torch.cat([_loc - _reg_targets[:, :2], _loc + _reg_targets[:, 2:]], dim=-1) # (N, 4)
        reg_p_x1y1x2y2 = torch.cat([_loc - _reg_pred[:, :2], _loc + _reg_pred[:, 2:]], dim=-1)
        ...
        if self.box_quality == "ctrness":
            ...
            if self.loc_loss_type == "ciou":
                cious = compute_ciou(pred=reg_p_x1y1x2y2, target=reg_t_x1y1x2y2)
                reg_loss = self.loc_loss_func(ious, gious, ctrness_targets, cious=cious) / loss_denorm
                # reg_loss = self.loc_loss_func(ious, gious, ctrness_targets, cious=cious) / num_pos_avg  # iou_3
                losses["loss_fcos_loc"] = reg_loss * 0.5
            else:
            ...
            if self.loc_loss_type == "ciou":
                cious = compute_ciou(pred=reg_p_x1y1x2y2, target=reg_t_x1y1x2y2)
                reg_loss = self.loc_loss_func(ious, gious, ctrness_targets, cious=cious) / num_pos_avg
                losses["loss_fcos_loc"] = reg_loss
            else:
